...
void * global_thread_init(mc_control::MCGlobalController::GlobalConfiguration & gconfig)
{
    ...
    {
        mc_rtc::log::error_and_throw<std::runtime_error>("[mc_kortex] mc_rtc cannot run faster than 1kHz with mc_kortex");
    }
    ...
    auto & kinovas = *loop_data->kinovas;
    {
        ...
        for(auto & robot : robots)
        {
                ...
                kinova_init_threads.emplace_back([&, ip, username, password]() {
                    {
                        ...
                        kinova_init_cv.wait(lock, [&kinovas_init_ready]() { return kinovas_init_ready; });
                    }
                    auto kinova = std::unique_ptr<mc_kinova::KinovaRobot>(new mc_kinova::KinovaRobot(robot.name(),ip,username,password));
                    std::unique_lock<std::mutex> lock(kinova_init_mutex);
            ...
            {
                mc_rtc::log::warning("The loaded controller uses an actuated robot that is not configured and not ignored: {}", robot.name());
            }
        }
    ...
    controller.controller().gui()->addElement({"Kortex"}, mc_rtc::gui::Button("Stop controller", [&controller]() { controller.running = false; }));
    ...
    for(auto & kinova : kinovas)
    {
        loop_data->kinova_threads->emplace_back([&]() { kinova->controlThread(controller, startMutex, startCV, startControl, controller.running); });
    }
    startControl = true;
    startCV.notify_all();
...
void run(void* data)
{
    ...
    controller.controller().logger().addLogEntry("perf_LoopDt", [&]() { return (now - last) / 1000; });
